...
#### 5.7.1 「버전 불일치」는 불일치가 아니었다 — 축이 셋이다 (2026-09-14 실측, 닫힘)

2026-09-11에 «§5.6 실측 `1.0.2` vs 공개 릴노트 최신 `v1.0.0`»을 **미해결**로 남기고 가설 둘을 적었다
(패치 릴노트 미공개 / 우리 필드 해석 차이). **두 번째가 맞았다.** 기기에서 세 축을 갈랐다.

| 축 | 값 | 어디서 나오나 | 무엇을 세는가 |
|---|---|---|---|
| **릴리스 라인(게이트)** | **`1.0.0`** | 피드 ipk 전부의 `Required-OS-Version` | 앱이 요구하는 **OS 세대**. 공개 릴노트의 `v1.0.0`이 이것이다 |
| **OS 베이스 버전** | **`1.0.2`** | `/etc/os-release VERSION_ID` = opkg `smhub-os-base` | 그 라인 안의 **패치 번호**. `1.0.2 ≥ 1.0.0` → 게이트 충족 |
| **RAUC 번들** | **`0.2.1`** | `rauc status --detailed` `BUNDLE_VERSION` | 커널/rootfs 번들 자체의 독립 버전 |

결정적 사실 둘 [측정 2026-09-14, 기기]:

- **`smhub-os-base`는 피드에 없다.** 캐시된 `smhub_core` 인덱스(14,282 B)를 전수 파싱해도 그 이름이
  안 나온다. 즉 **설치 가능한 패키지가 아니라 RAUC rootfs가 심는 마커**이고, 존재 이유는
  `Provides: smhub-os-base-1.0.2` 하나 — 다른 ipk의 `Required-OS-Version` 검사를 통과시키는 것이다.
- **게이트 값은 사다리를 이룬다**: `0.3.0` → `0.3.11` → `0.9.9` → **`1.0.0`**. 현재 앱 카탈로그가
  전부 `1.0.0`에 물려 있다. `smhub-web`만 예외로 아직 `0.3.0`이다.

→ **`v1.0.0`과 `1.0.2`는 같은 축의 어긋난 두 눈금이 아니라, 게이트 값과 그것을 만족하는 실제 값이다.**
벤더가 패치 릴노트를 안 내는 것도 이 구조에서는 일관적이다 — 패치가 올라도 **어떤 앱의 의존도 바뀌지
않으므로** 공지할 것이 없다. `docs/SMHUB.md:388`이 적어 둔 `Required-OS-Version: 1.0.0`이 이미 그
단서였는데, 그때는 그것을 「릴노트 표기와 필드명이 다르다」로만 읽었다.

⚠️ **남는 것 하나**: `1.0.1`·`1.0.2`에 *무엇이 바뀌었는지*는 여전히 모른다. 축이 갈렸을 뿐
변경 내용은 벤더 비공개다. 그건 «불일치»가 아니라 «미공개»이므로 §6 정보 벽으로 옮긴다.

#### 5.7.2 곁가지 — 기준 기기는 흘러가지 않는다, 그리고 카탈로그가 보인다 (2026-09-14 실측)

**(a) 매일 도는 opkg cron은 인덱스만 갱신한다. 업그레이드하지 않는다.**

```sh
# /etc/cron.daily/opkg — 전문이다
opkg clean
opkg update
# /etc/cron/crontabs: 0 3 * * * sleep $((RANDOM % 600)) && run-parts /etc/cron.daily
```

`/var/lib/opkg/lists/smhub_core`의 mtime이 `2026-09-14 03:09:44 UTC`인 것은 이 cron이지 사람이 아니다.
**`upgrade`가 없으므로 이 기기는 우리가 모르는 사이 패키지가 올라가지 않는다** — 대조 기준으로 쓸 수 있다.

**(b) `smhub-web`만 공장 빈티지로 남아 있다.**

| 패키지 | 설치본 | 설치 시각 | 피드 최신 |
|---|---|---|---|
| `smhub-broker` | 1.0.4-1 | 2026-09-07 | 1.0.4-1 |
| `smhub-services` | 1.1.0-1 | 2026-09-07 | 1.1.0-1 |
| `smhub-ui` | 1.0.6-1 | 2026-09-07 | 1.0.6-1 |
| `zigbee2mqtt` | 2.13.0-1 | 2026-09-07 | 2.13.0-1 |
| **`smhub-web`** | **0.3.1-1** | **2026-03-17** | **0.3.3-1** ← 2단계 뒤 |

`0.3.2-1`부터 `Depends: smhub-services (>= 0.2.13-1)`가 붙었다. 급하지 않다(우리는 Web UI를 제품
표면으로 안 쓴다). 다만 **`Required-OS-Version`이 `0.3.0`에 머문 유일한 패키지**라 위 사다리의 예외다.

**(c) 벤더 앱 카탈로그가 피드에 그대로 보인다** — `docs/ECOSYSTEM-PORTFOLIO.md`의 «벤더 앱-레지스트리
모델»에 실물 근거다.

```
matterbridge 3.9.4-2 · matterbridge-hass/-shelly/-z2m · openthread 2026.7.0-3
zwavejsui 11.21.0-1 · nodered 4.1.10-1 · tailscale 1.78.1-7 · picoclaw 0.2.8-2
esphome-bin 2026.5.3-5 (설치됨) · nodejs 22.22.0-2 (설치됨)
```

⚠️ **`openthread 2026.7.0-3`이 이 보드용으로 패키징돼 있다.** Nano는 MG24 하나뿐이라 Zigbee와
배타인데도 벤더가 OTBR 패키지를 낸다 — `docs/MULTIPROTOCOL.md`의 «언제 단일칩으로 가나» 축에
