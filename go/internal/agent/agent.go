...
func (a *Agent) buildEventPrompt(evt EventContext, devices []DeviceInfo) string {
	...
	sb.WriteString("- 이것은 이벤트 알림입니다. 질문이 아닙니다. '아니요'로 시작하지 마세요.\n")
	sb.WriteString("- 알림 형식: '[장소] [디바이스]이(가) [상태]' (예: '현관문이 열렸습니다')\n")
	sb.WriteString("- 22:00~06:00 사이 문 열림 → ⚠️ 경고 추가\n")
	sb.WriteString("- 낮 시간 → 간결한 한 줄 알림\n")
