...
|---|---|---|---|---|
| **현재 · 채택 ✅** | Zigbee2MQTT | **Node 22** | **디스크 141M** (node 49.5M + node_modules 92M) — RSS 아님 | 아니오 |
...
> **결론 방향 (2026-09-01 당시)**: 플랫폼 선택보다 **Zigbee를 Node에서 떼는 것**이 크다.
>
> 🚫 **이 방향은 채택되지 않았다 (RAIL 10, 2026-09-08).** §6.2가 Node 49.5M 자리에 CPython+zigpy
> 86M이 들어온다는 걸 재면서 "뗀다"의 이득이 사라졌고, GLG가 풋프린트를 판정 축에서 제외했다.
> **현행은 Z2M이다.** Node를 정말 빼고 싶어지면 목적지는 위 표 3행(**자체 Zig 게이트웨이**)이지
> Z4D가 아니다.
...
| **디스크 소계** | **43 M** | **141 M** |
| RSS | **121 M** (측정, x86) | **93.0 M** — [측정 2026-09-08, SMHub riscv64 실기, 유휴·페어링 0대] |

> **디스크는 이겼다 — 1/3.3, 98M 절감.** **RAM은 아직 판정 못 한다.**
> **[2026-09-08 갱신]** Z2M+Node의 RSS는 이제 있다 — SMHub riscv64 실기에서 **93.0 M**(유휴,
> paired 0대). 다만 아래 121MB는 **x86 + Z4D**라 아키텍처도 스택도 달라서 **동등 대조군이
> 아니다**; 방향의 참고로만 읽어라. 아래 원문은 2026-09-01 시점 기록이다.
>
...
| domoticz + **Z2M** ✅채택 | 35 M (x86) · **23.6 M**(riscv64 실측) | + Node **93.0 M**(riscv64 실측) | **116.6 M** — [측정 2026-09-08, SMHub 실기]. **보드 domoticz를 제거한 2026-09-10 이후 운용값이 아니라 닫힌 이식성 증명 스냅샷** |
...
> 2026-09-08 갱신. RAIL 10(Z2M 채택)과 실기 측정으로 여러 항목이 **취소되거나 닫혔다.**

1. ~~최신 `cryptography`로 Z4D가 도는가~~ → **취소.** Z4D 비채택(RAIL 10). 이 질문은 값이 없다.
2. ~~domoticz `2026.3` Buildroot 레시피~~ → **닫힘 (2026-09-08).** `SITE_METHOD=git` +
   `GIT_SUBMODULES=YES`로 서브모듈 5개 조달, riscv64 크로스빌드·ipk 패키징까지 완료
   (`smhub/`, `smhub/RUNBOOK.md`). 새 Buildroot 패키지 0개.
3. ~~domoticz 바이너리 실측~~ → **닫힘: 17.4 MB** (x86_64, §6.1) / **14.9 MB** (riscv64, 2026-09-08).
4. ~~RSS 분해~~ → **닫힘: 35 M(domoticz) + 86 M(Python) = 121 M** (§6.2, x86).
   - (a) **Z2M 대조군** → **닫힘 (2026-09-08, SMHub riscv64 실기)**: z2m idle **93.0 M**,
     domoticz **23.6 M**. 단 **paired 0대·한 유닛·유휴**이고 x86 Z4D와는 동일 workload가 아니다.
   - (b) **기기 수에 따른 증가 곡선** ← **여기가 남은 진짜 값이다.** paired 30~40대.
   - (c) ~~riscv64에서 다시~~ → 닫힘(위 a). **musl은 여전히 미측정.**
5. ~~Z4D의 아웃바운드 셋을 끌 수 있는가~~ → **취소.** Z4D 비채택. (오프라인 가부는 이제 Z2M
   축의 질문이고, Z2M엔 등가물이 없다.)
6. **musl 가부** — 제품 ISA 레인. **riscv64/glibc는 완료**(2026-09-08). musl 미측정.
7. **`ser2net`로 EZSP 원격 구동 시 지연·안정성** — A⁰의 유일한 미지수. 그대로 열림.
8. **SMHUB OS 프로파일** — 우리 기기는 **`1.0.2`**(2026-09-07 OTA, 2026-09-08 실측). 위 값은
   전부 그 한 유닛의 것이고, 다른 유닛/프로파일은 재검증이 필요하다.
