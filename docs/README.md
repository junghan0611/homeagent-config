...
HomeAgent is a **minimal open hub BSP** project first — a reproducible verification/prototyping ground with **no business logic**. The core lane is **Milk-V Duo S (SG2000)**, booted on real silicon from our own Buildroot image (full-stack ownership). SG2000 carries both an A53 and a C906 on one die and a **physical slide switch** picks which boots — **and the board in hand picks the ISA.** Today that board is the **SMHUB Nano** (equipped on arrival, least hand-work), so the work is **riscv64**. **arm64 on Duo S is a driven detour taken for Node.js alone** — Buildroot's `nodejs` supports `BR2_aarch64` and not riscv64 — and it reached flash-and-go in **v2026.7.24**. `riscv64-linux-musl` was always the product ISA and runtime target; Duo S rejoins it once Node builds for riscv64. The runtime design — **Zig 100ms state machine on Linux + a C906L FreeRTOS mailbox coprocessor base** (see `../runtime/README.md`) — is unchanged; radio is a **USB ZBDongle-E**. **SMHUB Nano MG24** is the commercial reference (vendor OS, system-application approach, onboard EFR32MG24; single MG24 means Zigbee *or* Thread, exclusive). Tuya THP23-ZB-X is parked as 128MB lower-bound evidence. RPi5/Yocto/Hailo, OPi5, and Android/RK work remain preserved origin or compatibility evidence.
