...
**남은 일**: `tune.sh`에 ASH 관측 모드(`info` + `log_output` file 한 쌍)를 넣어 소유권을 되돌린다.
지금은 기기와 스크립트가 어긋나 있다 — 누가 `tune.sh`를 돌리면 `log_level`만 되돌아가고
`log_output`은 남아 **반쯤 되돌아간 상태**가 된다.
