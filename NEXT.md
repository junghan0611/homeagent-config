...
- **벤더 매뉴얼 검토 진행중**: `docs/SMHUB-MANUAL-REVIEW.md` (공식 22페이지). **①완료 (2026-07-01)**: B-6 Radios&Protocols + C Zigbee2MQTT→HA 대조 → 결과 `docs/PRODUCT-CONFIG-MODEL.md §6b`. 핵심: **B-6은 시리즈 공통 제네릭 문서**(별도 칩 CC26xx/EFR32MG/EFR32ZG23·동시운용 전제) → Nano Mg24=단일 MG24(ember) 미적용, §6이 정본. z2m 업데이트=설치성 mutation(§8 게이트). **다음 = ②A Update/Restore Methods + External SSH**(OTA/백업 게이트 절차 정본화) → ③B-2/4/7/8 → ④C Peripheral conf. 각 페이지 "벤더 절차 ↔ 라이브 실측" 대조.
