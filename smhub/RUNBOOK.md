...
### 2.6.6 ⚠️ 새 비밀번호를 만들었으면 **영속 저장이 절차의 일부다**

이 절이 이걸 안 적어서 한 번 날렸다. 2026-09-10에 재설정하고 평문을 `/tmp`에만 뒀는데,
`/tmp`가 **tmpfs**라 2026-09-16 재부팅과 함께 사라졌다 — 그리고 argon2id는 단방향이라
DB에서 되찾을 수 없다. **다시 만드는 것 말고는 길이 없었다.**

```
해시 교체 → 로그인 200 확인 → 평문을 영속 저장소에 쓴다   ← 이 세 번째가 절차다
```

저장 자리는 리포 루트 **`.env.smhub`**(gitignored, `chmod 600`), 키는 `SMHUB_WEBUI_PASS`.
좌표와 키 목록은 `PRIVATE.md` §🔑. ⛔ `/tmp`·세션 로그·추적 파일에 평문을 남기지 마라.

**그리고 이 두 가지를 더 밟았다** (2026-09-16):

- `sqlite3`에서 `"..."`는 문자열이 아니라 **식별자**다. 해시를 `"`로 감싸면 `no such column`으로
  죽는다. 셸 중첩 따옴표까지 겹치면 더 깨진다 → **기기에 `.sh`로 올려** `printf`로 SQL을 만들고
  `sqlite3 "$DB" < file`로 먹여라. (§B의 「`$`가 셸에서 확장된다」와 별개 함정이다.)
- **재부팅 직후엔 Web UI가 502를 준다.** `smhub-services`가 부팅 등록이 아니라 nginx가 뜰 때
  `/run/smhub-backend.sock`이 없었기 때문이다. **백엔드 문제가 아니다.** 판정과 복구:

  ```sh
  curl -s --unix-socket /run/smhub-backend.sock http://localhost/api/v1/users/me   # 200이면 백엔드 정상
  sudo nginx -s reload                                                              # 502가 사라진다
  ```
...
### 6.5.0 ⛔ 다시 세우지 마라 — 철회된 프레임 셋 (2026-09-09, 2026-09-16 갱신)

이 셋은 한 번씩 그럴듯하게 서 있다가 측정으로 무너졌다. 크래시를 다시 볼 때 **여기부터 읽어라.**

1. **「hw_flow를 구우면 반드시 침묵한다」** — 철회. `rtscts:false`는 호스트 설정이지 MG24의
   CTS 전기 레벨이 아니다. 위험은 *필연*이 아니라 **미측정 배선/극성에 걸린 도박**이다.
2. **「no_flow라서 RX 오버런이 난다」** — 철회. `oe:`가 계속 **0**이다. 바이트는 한 번도 안 흘렸다.
   flow control·DMA·펌웨어 버전은 이 실패의 자리에 **없다.**
3. **「14대 부하 / 인터뷰 버스트가 CPU를 굶긴다」** — 절반 철회(2026-09-09), 그리고
   **2026-09-16에 더 약해졌다.** 부하 0인 `GET_EUI64` 초기화 지점에서도 죽었고,
   43시간 무접촉 정상 운전은 **완전히 깨끗했다**(덤프 46개, `rxAckTimeouts` 전부 0).
   기록된 크래시 둘은 **둘 다 재기동 직후**다(+8분14초, +18초).
   → 지금 살아 있는 모양은 「부하가 굶긴다」가 아니라 **「어댑터 (재)초기화 구간」**이다. ⚠️ n=2.
4. **(2026-09-16 신규) 「`rxAckFrames=0`은 크래시 증상이다」** — 철회. 이 NCP는 standalone ACK를
   안 보내고 piggyback한다. **정상 구간도 0이다.** 이 숫자를 근거로 쓰면 틀린다.

⚠️ 그리고 **2026-09-09의 「완전 파워사이클이 루프를 끊었다」는 *진짜 루프*에만 해당한다.**
한 번 나고 자동 복구된 크래시에 진단 목적으로 파워사이클을 하지 마라 — §6.5.2a 이후로
재기동 자체가 용의선상이고, 콘솔 로그가 tmpfs라 증거도 같이 날아간다.
...
### 6.5.2a-보정 두 표본은 V8 설정도 다르다 — 그런데 모양은 같았다 [측정 2026-09-14 16:50]

§6.5.2a를 쓸 때 두 크래시의 차이를 «17대 페어링 vs 2대 유휴»로만 적었다. **한 축을 빠뜨렸다.**

| | 2026-09-08 크래시 | **2026-09-14 크래시** |
|---|---|---|
| `NODE_OPTIONS` | **없었다** — `/etc/conf.d/zigbee2mqtt`가 그날 **08:27**에 생겼고 크래시는 **08:06**이다 | **있다** (`--v8-pool-size=0 --max-old-space-size=128 --max-semi-space-size=2`) |
| V8 `heap_size_limit` | **259 MB** (기본) | **134 MB** |
| `adapter_concurrent` | 2 | 1 |
| 기기 / 상태 | 17대 · 페어링 버스트 | 2대 · 유휴 |

`heap_size_limit`은 실측했다 — 같은 바이너리에 옵션을 넣고 뺐다:

```
NODE_OPTIONS=…  /opt/bin/node -p v8.getHeapStatistics().heap_size_limit  →  134 MB
(옵션 없이)      같은 명령                                                →  259 MB
```

→ **§6.5.2의 «259 MB» 서술은 그 크래시에는 맞다** (그때는 conf.d가 없었다). 다만 **현재 운용값은
134 MB**이므로 그 문단을 현재값으로 읽으면 틀린다.

**읽을 것**: 09-08 이후 우리가 넣은 완화 셋(`adapter_concurrent` 내림 · `NODE_OPTIONS` · V8 워커
스레드 제거)을 **전부 적용한 상태에서도 같은 모양으로 죽었다.** 특히 `--v8-pool-size=0`은 1코어
경합을 줄이려던 것인데 실패 모드가 안 바뀌었다. 이 고장은 **대수·부하·V8 구성 셋 다 건너뛴다.**

⚠️ 다만 힙을 259→134로 **조이면 GC는 더 자주 돈다.** 그러니 이것을 「GC 가설 반증」으로 읽으면
안 된다 — 방향이 반대인 두 변화가 한 옵션에 섞여 있다.

### ⚠️ 6.5.2c 이 실험에서 우리가 계약을 하나 어겼다 — `tune.sh` 우회

**`smhub/tune.sh`가 이 값들의 소유자다** — `log_level` · `log_output` · `adapter_concurrent` ·
`NODE_OPTIONS` 넷 다. `--show` / `--revert`를 갖고 있고 멱등이다.

**2026-09-14 ASH 1단계는 그 스크립트를 안 쓰고 `configuration.yaml`을 `sed -i`로 직접 고쳤다.**
파일 소유자(`smlight:smlight`)는 지켰지만 **값의 소유자를 건너뛴 것**이고, 이 리포가
`4043184`에서 이미 한 번 배운 것과 같은 계열이다(그때는 소유자가 z2m, 이번엔 우리 스크립트).

구체적 결과 둘:

1. **의도를 하나 덮었다.** `tune.sh:43`이 *"Kept `log_output` at `[console]` so the path does not
   move"* 라고 **일부러** 적어 뒀는데, 그 판단을 못 보고 `file`을 더했다.
2. **기기와 스크립트가 어긋나 있다.** 지금 누가 `tune.sh`를 돌리면 `log_level`은 `warning`으로
