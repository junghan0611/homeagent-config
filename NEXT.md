...
### 🔴 우리 노드 이름이 바뀌었다 — `gq-node-02` → **`gq-node-02-r1`** (2026-09-14)

옆 레인이 GLG 지시로 마스터 DB와 `master/nodes.json` 둘 다 바꿨다. **리허설 없이 갔다**(내가
「우리 먼저」를 제안했으나 GLG가 직접 지시). 내가 그쪽 리포와 마스터 DB를 **직접 읽어 확인했다**:

```
Hardware(2) gq-node-01-r1  192.168.0.192:1883  homeassistant-1   145개
Hardware(4) gq-node-01-r2  192.168.0.192:1883  homeassistant-2     4개
Hardware(3) gq-node-02-r1  192.168.0.124:1883  homeassistant      22개   ← 우리
```

**`HardwareID=3` 보존 · 22개와 이력 그대로 · `discoveryPrefix`는 `homeassistant` 유지**
(브로커가 다르므로 바꿀 이유 없다는 우리 판정이 그대로 반영됐다). **보드는 안 건드려졌다.**

⚠️ 이 문서 아래쪽 09-10 기록의 `gq-node-02`는 **그 시점의 이름**이라 고치지 않는다.
지금 이름은 이 절이 정본이다.
...
ASH 관측을 켠 지 **8분 21초 만에** 이슈 #11의 그 실패가 났다.
`ERROR_EXCEEDED_MAXIMUM_ACK_TIMEOUT_COUNT` → `ASH_NCP_FATAL_ERROR`, 25초 뒤 자동 재기동,
`matches config`로 복귀, 이후 재발 없음. 전문·카운터·비교표는 **`smhub/RUNBOOK.md` §6.5.2a**.

읽을 것 하나: **§6.5.2(17대 페어링 중)와 부피만 다르고 모양이 같다** — ACK TX 858 vs 93,
Retry dupes 20 vs 6, 모든 오류 카운터 0, Last Frame 둘 다 `SEND_UNICAST`.
**이 고장은 양에 비례하지 않는다.** RAIL 11의 「N대에서 죽는다」 곡선 전제가 흔들린다.

⚠️ **원인 미확정. 혼입 둘 다 우리 손이다** — `log_level: info` 전환(8분 전)과 에이전트
SSH 프로빙(92초 전). 직전 4일은 무접촉·`warning`·크래시 0이었다. n=1이라 **「로깅 때문」으로
적지 않는다.** 가르는 법 = `info` 유지 + **SSH 끊고 24시간 방치**(혼입이 하나로 준다).

### ✅ 첫 매시간 CSV — 나왔다, 오류 0 [측정 2026-09-14 16:09:24 KST]
...
정상 운전 1시간 · 2대 · **오류 카운터 합계 0.** ASH 1단계가 닫으려던 질문
「정상 운전에서 1코어가 ASH 오류를 내는가」는 **이 구간에서는 안 낸다**로 답했다.
필드 순서·해독·비교표는 `smhub/RUNBOOK.md` **§6.5.2b**.

읽을 것 셋:

1. **x86과 프레임률이 같다** — 우리 192 프레임/시간/기기 vs 저쪽 179 [인계, 그쪽 114h 기준선].
   아침의 이벤트율 대조(0.83 vs 0.80 건/분/기기)와 **독립적으로 같은 결론**이다.
2. 🔍 **`rxAckFrames=0`은 크래시 증상이 아니었다.** 정상 구간도 0이다 — 이 NCP는 standalone ACK를
   안 보내고 piggyback한다. §6.5.2의 `ACK frames RX=0, TX=858`을 「하나도 못 받았다」로 읽으면
   틀린다. **이슈 #11에 그 숫자를 근거로 썼다면 그 줄은 근거가 못 된다.**
3. **전조가 없다.** 정상 구간은 완벽히 깨끗하고 죽을 때만 `ACK timeouts`가 찍힌다 —
   열화가 아니라 **멀쩡하다가 한 번에 가는** 모양이다.

⚠️ 여전히 `adapter_concurrent: 1`이다. 2단계 대조 없이 「1코어라서」로 일반화하지 않는다.

### ⚠️ 이 실험에서 우리가 두 가지를 어겼다 (2026-09-14 16:50 자기점검)

1. **`smhub/tune.sh`가 이 값들의 소유자인데 우회했다.** `log_level`·`log_output`·
   `adapter_concurrent`·`NODE_OPTIONS` 넷 다 그 스크립트 것이고 `--show`/`--revert`가 있다.
   나는 `configuration.yaml`을 `sed -i`로 직접 고쳤다. 파일 소유자는 지켰지만 **값의 소유자를
   건너뛰었다.** 게다가 `tune.sh:43`이 *"Kept `log_output` at `[console]` so the path does not
   move"* 라고 **의도를 적어 뒀는데** 그걸 못 보고 `file`을 더했다. → `RUNBOOK.md` §6.5.2c
2. **이 문서가 혼입을 이미 경고하고 있었다** — 아래 튜닝 문단: *"14대급에서 `info`는 그 자체가
   CPU 부하라 상시로 두지 않는다."* 켜기 전에 안 읽었다. §3.5의 「로깅이 혼입」은 내 추측이
   아니라 **이 리포가 먼저 적어 둔 것**이다.

3. **그런데 다시 재 보니 로깅 혼입은 약해졌다** [측정 17:05] — 크래시 직전 30초 **32줄**(대부분
   크래시 자신의 카운터 덤프), 평범한 30초 창은 **0줄**. `tune.sh`의 「89줄/30초」는 **17대 페어링
   버스트** 값이고 2대 유휴엔 그 조건이 없다. → **로깅이 루프를 굶겼다는 설명은 오늘 크래시에
   안 맞는다.** 남는 혼입은 SSH 프로빙 하나이고 그것도 확정이 아니다. 상세 §6.5.2a-보정2.
4. **다만 `log_output: file` 한 조각은 실제로 계약을 깼다.** 콘솔은 `/tmp`(**tmpfs**)로 가는데
   file은 `/mnt/user`(**eMMC**)다 — `tune.sh`가 *"so the path does not move"* 로 지킨 게 그 tmpfs다.
   지금은 1.7건/분이라 미미하지만 **페어링 버스트에선 89줄/30초가 eMMC로 간다.**
