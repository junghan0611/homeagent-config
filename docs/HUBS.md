...
> **상태 (2026-07-14)**: **DIRIGERA 레인 폐기** — 구매·진행 안 함. 이 문서는 이제 **랜드스케이프
> 조사 자료로만** 남으며 **우리 방향이 아니다**(현재 방향 = Duo S RISC-V 코어 레인 + SMHub 상용
> 레퍼런스). 본문의 "주력/전면 타깃" 표현은 조사 시점 프레이밍이며, 현재 스탠스는 이 배너가 우선한다.
...
**IKEA DIRIGERA = (조사 시점) 주력 후보 — 現 레인 폐기(위 상태 배너).** 인증됐고, Linux 기반이라 손댈 여지가 크며, 아키텍처가 SG2000 허브와
